...
- **Assembly files require “why” comments.** If an instruction sequence is subtle, explain the reason and the consequence of getting it wrong.
...
- **Architecture acronyms must be expanded.** If you use `ERA`, `ESTAT`, `CRMD`, etc., include a short mapping to the CSR name and role nearby. Prefer using expanded names in C++ code (e.g., `exception_return_address` instead of `ERA`) in the vast majority of cases.
- **Public interfaces should be Doxygen-friendly.** Headers should document intent, inputs/outputs, invariants, and ordering constraints.
...
- **Assume freestanding constraints.** No OS, no libc expectations. Avoid accidental dependencies.
...
- Self-tests are encouraged when they **prove a property** (e.g., “ERTN return path works”, “timer interrupt delivery works”).
